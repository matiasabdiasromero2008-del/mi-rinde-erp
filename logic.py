...
def recalculate_product_gpu(product_id, cursor):
    ...
    cursor.execute("UPDATE products SET current_gpu = ? WHERE id = ?", (gpu, product_id))
...
def add_expense(provider, category_name, items_list, date_str):
    ...
    for item in items_list:
        ...
        if category_name == "INSUMOS":
            cursor.execute("UPDATE ingredients SET last_unit_cost = ? WHERE name = ?", 
...
def record_sale(client_name, items_list, discount, date_str=None):
    ...
    valid_items = []
    for item in items_list:
        cursor.execute("SELECT flavor_name, sale_price, current_gpu FROM products WHERE id = ?", (item['product_id'],))
        prod = cursor.fetchone()
        
        cursor.execute("SELECT quantity_remaining FROM stock WHERE product_id = ?", (item['product_id'],))
        stock_qty = cursor.fetchone()[0]
    ...
    for item in valid_items:
        ...
        cursor.execute("UPDATE stock SET quantity_remaining = quantity_remaining - ? WHERE product_id = ?", 
                       (item['qty'], item['id']))
...
def get_performance_metrics(month_str):
    ...
    ingresos = sales_data[0] or 0
    gtr = sales_data[1] or 0
